...
namespace VkBotApi
{
    public class Totp
    {
        ...
        public int GetCode(ulong timeStamp)
        {
            var message = timeStamp / (ulong)_t1;
            var msgByte = BitConverter.GetBytes(message);
            if (BitConverter.IsLittleEndian)
                Array.Reverse(msgByte, 0, msgByte.Length);
            var hash = _hmac.ComputeHash(msgByte);
            var offset = (hash[hash.Length - 1] & 0xf);
            var i = ((hash[offset] & 0x7f) << 24) | ((hash[offset + 1] & 0xff) << 16) | ((hash[offset + 2] & 0xff) << 8) |
                    (hash[offset + 3] & 0xff);
            var code = i % (int)Math.Pow(10, _digits);
            return code;
        }
        ...
        public string GetCodeString(ulong timeStamp)
        {
            var ret = GetCode(timeStamp) + "";
            return ret.PadLeft(_digits, '0');
        }
